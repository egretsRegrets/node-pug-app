...
function autocomplete(input, latInput, lngInput) {
    ...
    // see layout in views for inclusion call to this google api
    // the following methods use syntax spec to the googlem aps api
...
function clientFlash(msg) {
    var html = '<div class="flash-messages">\n        <div class="flash flash--error">\n            <p class="flash__text">' + msg + '</p>\n            <button class="flash__remove">&times</button>\n        </div>\n    </div >';

    (0, _bling.$)('.inner').innerHTML = html;
    (0, _bling.$)('.flash__remove').addEventListener('click', function () {
...
function drawMap(options) {
    var map = new google.maps.Map((0, _bling.$)('#map'), options);
    loadPlaces(map);
...
function typeAhead(search) {
    ...
    searchInput.on('input', function () {
        ...
        // on is bling method for addEventListener
        ...
        _axios2.default.get('/api/v1/search?q=' + this.value).then(function (res) {
            if (res.data.length) {
                ...
                searchResults.innerHTML = _dompurify2.default.sanitize('<div class="search__result">No results found for <strong>' + _this.value + '</strong></div>');
            }
        }).catch(function (err) {
            console.error(err);
        });
...
var heartForms = (0, _bling.$$)('form.heart'); // $$ is querySelectorAll
